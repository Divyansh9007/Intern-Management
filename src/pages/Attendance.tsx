...
const Attendance = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!isAdmin) {
      toast.error('Only admin can mark attendance');
      return;
    }
  ...
  const displayInterns = user?.role === 'admin' ? interns : interns.filter(intern => intern.id === user?.id);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            {isAdmin ? 'Track and manage intern attendance' : 'View your attendance records'}
          </p>
        </div>
        {isAdmin && (
          <button
      ...
      {/* Mark Attendance Modal - Only for Admin */}
      {isAdmin && (
