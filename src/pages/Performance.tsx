...
const Performance = () => {
  ...
  // Only admin can add performance reviews
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!isAdmin) {
      toast.error('Only admin can add performance reviews');
      return;
    }
  ...
  const displayPerformances = user?.role === 'admin' 
    ? performances 
    : performances.filter(perf => perf.internId === user?.id);
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            {isAdmin ? 'Manage intern performance reviews' : 'View your performance reviews'}
          </p>
        </div>
        {isAdmin && (
          <button
      ...
      <div className="grid gap-6">
        {displayPerformances.length > 0 ? (
          displayPerformances.map((review) => {
              ...
              <div key={review.id} className="bg-white rounded-lg shadow p-6">
                ...
                <div className="mt-4 grid grid-cols-2 gap-4">
                  ...
                  <div className="bg-gray-50 rounded-lg p-4">
                    ...
                    <p className="text-2xl font-semibold mt-1">{new Date(review.lastReview).toLocaleDateString()}</p>
                  </div>
                </div>
          ...
          <div className="bg-white rounded-lg shadow p-8 text-center">
            ...
            <p className="text-gray-500">
              {isAdmin ? 'Start by adding performance reviews for your interns.' : 'No performance reviews available yet.'}
            </p>
          </div>
        )}
      </div>

      {/* Add Performance Review Modal - Only for Admin */}
      {isAdmin && isModalOpen && (
